...
const connection =con.connect(function(err){
    ...
    let sql = "select questions.que_id, questions.que, questions.code, questions.answer, GROUP_CONCAT(options.choice_value,')' , options.choice SEPARATOR '<br>') as hello from questions left join options on questions.que_id = options.que_id group by questions.que_id;";
...
const server=http.createServer(function(req, res){
    ...
    for(let i=0; i<que1.length;i++){
       ...
       // result1[i]= "Question ID"+que1[i].que_id +":"+que1[i].que+ "<br>"+que1[i].code+"<br>";
            result1[i]= "Question ID"+que1[i].que_id +" "+que1[i].que+ "<br>"+que1[i].code+"<br>"+que1[i].hello+"<br>"+"Answer is "+que1[i].answer+"<br>";
